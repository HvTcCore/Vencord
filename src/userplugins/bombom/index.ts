...
const getFrames = makeLazy(() => Promise.all(
    Array.from(
        { length: FRAMES },
        (_, i) => loadImage(`https://raw.githubusercontent.com/HvTcCore/File-O-Trash/main/VenShit/UsPl/bombom/frames/bom${i}.gif`)
    ))
);
        ...
        switch (opt.name) {
            ...
            case "user":
                try {
                    ...
                    console.error("[bombom] Failed to fetch user\n", err);
                    UploadManager.clearAll(ctx.channel.id, DraftType.SlashCommand);
...
export default definePlugin({
    name: "bombom",
    description: "Adds a /bombom slash command to create headbom gifs from any image",
    authors: [Devs.Ven, Devs.u32],
    commands: [
        {
            inputType: ApplicationCommandInputType.BUILT_IN,
            name: "bombom",
            description: "Create a bombom gif. You can only specify one of the image options",
            options: [
                {
                    name: "delay",
                    description: "The delay between each frame in ms. Rounded to nearest 10ms. Defaults to the minimum value of 30.",
            ...
            execute: async (opts, cmdCtx) => {
                ...
                const file = new File([gif.bytesView()], "bombom.gif", { type: "image/gif" });
